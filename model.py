...
class SSE(nn.Module):
    ...
    def gen_a_update(self,x_a):
        ...
        # Encode
        h_a,n_a,mag_a, real_a, imag_a = self.gen_a.encode(x_a) 
        # Decode
        x_a_recon,mag_a_recon = self.gen_a.decode(h_a+n_a,real_a)
        # Loss
    ...
    def gen_b_update(self,x_b,notnoise):
        ...
        # Encode
        h_b,n_b,mag_b,real_b, imag_b = self.gen_b.encode(x_b)
        # Decode (within domain)
        x_b_recon, mag_b_recon = self.gen_b.decode(h_b+n_b,real_b)
        # Decode (cross domain)
        x_ba, mag_ba = self.gen_a.decode(h_b+n_b,real_b)
        # Encode again
        h_b_recon,n_b_recon,mag_b_recon,real_b_recon, imag_b_recon = self.gen_a.encode(x_ba)
        # Decode again
        x_bab, mag_bab = self.gen_b.decode(h_b_recon+n_b_recon,real_b_recon)
...
class Conv1dBlock(nn.Module):
    ...
    def forward(self, x):
        #print("Conv1D before squeezing: \t", x.shape, "\n")
        x = torch.squeeze(x, -1)
        #print("Conv1D before convolution: \t", x.shape, "\n")
        x = self.conv(x)
        #print("Conv1D after convolution: \t", x.shape, "\n")
        if self.normalize:
            x = self.norm(x)
        x = F.softplus(x)
        return x
...
class ConvTranspose1dBlock(nn.Module):
    ...
    def forward(self, x):
        #print("ConvTranspose1D before convolution: \t", x.shape, "\n")
        x = self.conv(x)
        #print("ConvTranspose1D after convolution: \t", x.shape, "\n")
        if self.normalize:
            x = self.norm(x)
        x = F.softplus(x)
        return x
...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, stride, padding, normalize=True):
        # function requires tuples but config.yaml returns lists
    ...
    def forward(self, x):
        #print("Conv2D before convolution: \t", x.shape, "\n")
        x = self.conv(x)
        #print("Conv2D after convolution: \t", x.shape, "\n")
        if self.normalize:
            x = self.norm(x)
        x = F.softplus(x)
        return x
...
class ConvTranspose2dBlock(nn.Module):
    ...
    def forward(self, x):
        #print("Tensor before unsqueezing: ", x.shape, "\n")
        x = x.unsqueeze(3)
        #print("ConvTranspose2D before convolution: \t", x.shape, "\n")
        x = self.conv(x)
        #print("ConvTranspose2D after convolution: \t", x.shape, "\n")
        if self.normalize:
            x = self.norm(x)
        x = F.softplus(x)
        return x
...
class Encoder(nn.Module):
        ...
        if config['conv_dim'] == 1:
            if customize:
                for i in range(len(inter_dims)):
                    if i == 0:
                        self.layers.append(Conv2dBlock(int(self.filter_length/2)+1,inter_dims[0],\
                                       config['time2']['kernel_size'],config['time2']['stride'],config['time2']['padding']))
                    else:
                ...
                for i in range(config['num_layers']):
                    if i == 0:
                        self.layers.append(Conv2dBlock(int(self.filter_length/2)+1,self.dim,config['time2']['kernel_size']\
                                         ,config['time2']['stride'],config['time2']['padding']))
                    else:
    ...
    def forward(self,x): 
        real, imag = self.stft(x) # obtain real and imaginary part from STFT
        
        real_square = torch.square(real)
        imag_square = torch.square(imag)
        a = torch.sqrt(real_square + imag_square)

        real = torch.div(real, a)
        imag = torch.div(imag, a) # reduce dynamic range; values between +1 and -1
        ...
        return x, a, real, imag # return latent complex, magnitude, real part, imaginary part
...
class Decoder(nn.Module):
        ...
        if config['conv_dim'] == 1:
            if customize:
                for i in range(len(inter_dims)-1,-1,-1):
                    if i == 0:
                        self.layers.append(ConvTranspose2dBlock(inter_dims[i],int(self.filter_length/2)+1\
                                         ,config['time2']['kernel_size'],config['time2']['stride'],config['time2']['padding']))
                    else:
                ...
                for i in range(config['num_layers']):
                    if i == config['num_layers']-1:
                        self.layers.append(ConvTranspose2dBlock(self.dim*(2**i),int(self.filter_length/2)+1\
                                         ,config['time2']['kernel_size'],config['time2']['stride'],config['time2']['padding']))
                    else:
    ...
    def forward(self,x, p):
        ...
        x = self.tconv(x) # obtain stacked complex number
        real = x[...,0]
        imag = x[...,1]

        real_square = torch.square(real)
        imag_square = torch.square(imag)
        a = torch.sqrt(real_square + imag_square) # calculate magnitude from real and imaginary parts
        
        ph = torch.atan(imag / (real+EPS)) # calculate phase from real and imaginary parts
        
        x_recon = self.istft(a,ph)
